 ...
 * implementations can never drift. Locked IA: Accueil · Séances · Disponibles (3 tabs, per WBS).
 * Revenus is NOT a tab — the WBS coach bottom nav is 3 items (Home · Available · Planned); the
 * financial dashboard (C35) opens as a modal from Home's "Earnings" button instead.
 */
import React, { type ComponentType, type ReactNode } from 'react';
import type { SFSymbol } from 'sf-symbols-typescript';
import { Home, Calendar, Search } from '../icons';
...
export type TabName = 'Accueil' | 'Seances' | 'Disponibles';

export type TabDef = {
  name: TabName;
  /** Visible tab label — from the localization seam (../copy). */
  label: string;
